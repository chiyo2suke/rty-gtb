...
redips.init = function () {
  ...
  rd.event.dblClicked = function () {
    ...
    if (radioButton[0].checked && currentCell.classList.contains("b2")) {
      rd.moveObject({
        obj: rd.obj,
        target: originCell,
        callback: function () {
    ...
    if (b2Cell[i].children.length > 0) {
      for (var j = 0; j < b2Cell[i].children.length; j++) {
        ...
        if (thisRank.startsWith("Ms") || thisRank.startsWith("Sd")) {
          ...
          thisChg =
            '<span href="https://sumodb.sumogames.de/Query.aspx?show_form=0&form1_rank=' +
            thisRank +
            "&form1_wins=" +
            rikishiWins / 2 +
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var basho = "202503"; // The date of the basho just ended
  ...
  const banzuke1Config = [
      ...
      { divider: true },
      { prefix: 'Sd', range: Array.from({length: 3}, (_, i) => i + 1) }
  ];
  ...
  if (window.localStorage.getItem("savedBanzuke") !== null) {
    var saveDate = Date.parse(window.localStorage.getItem("savedBanzukeTime")),
      expireDate = new Date(Date.UTC(2025, 2, 23, 12, 50)); //UTC time
  ...
  if (radioLocal === null || radioLocal == "returnToOld")
    radioButton[0].checked = true;
  else if (radioLocal == "doNothing") radioButton[1].checked = true;
  //else radioButton[2].checked = true;

  if (radioLocalDrop === null || radioLocalDrop == "multiple")
    radioButton[2].checked = true;
  else if (radioLocalDrop == "shift") radioButton[3].checked = true;
